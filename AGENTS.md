...
7. User pulls `main`, tags the merge commit, and pushes the tag. The Release workflow checks the tag and release records, builds binaries, creates the GitHub release from `docs/release/vX.Y.Z.md`, and publishes to crates.io. Do not create the release by hand. A published crate version can only be yanked, never replaced.
