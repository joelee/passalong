...
| `key_file` | path | `store.key` beside the default config file | Where this device keeps an encrypted store's key: `$XDG_CONFIG_HOME/passalong/store.key`, or `~/.config/passalong/store.key`, or on Windows `%APPDATA%\passalong\store.key`; there is no default when neither `XDG_CONFIG_HOME` nor `HOME` is set. Must be absolute after `~` expansion. It is written with mode 0600, and refused if other users can read it or if it is inside a git work tree that does not ignore it |
