---
title: "Idea 00001 r03: Encryption At Rest"
aliases:
  - "Idea 00001"
tags:
  - idea
...
created: 2026-09-15
updated: 2026-09-15
analysed_at: "2026-09-15T08:09:15Z"
agent: "Claude Code"
model: "anthropic/claude-opus-5"
triggered_by: user
previous_revision: "[[00001-Encryption_At_Rest-r02]]"
...
# Idea 00001 r03: Encryption At Rest

> [!abstract] Recommendation: `proceed-to-experiment`
> The user has now settled every design question from r02: data key
> rotation is in v0.1.7, `encrypt` may start a fresh encrypted store instead
> of migrating, passphrases are generated words, a key file inside a git work
> tree is refused, and Argon2id uses 64 MiB. One Major finding remains open:
> keyless and pre-0.1.7 clients must be unable to write plaintext into an
> encrypted store, which the spike in section 14 tests with the real v0.1.6
> binary. Rotation adds one new Medium risk, clients still holding the old
> data key, which the design closes by checking the store's key id before
> every write.
...
- Six-word generated passphrases from the EFF large word list.
...
#### IDEA-00001-R03-MAJ-01: Keyless and old clients can write plaintext into an encrypted store
...
#### IDEA-00001-R03-MED-01: The streaming AEAD construction is easy to get subtly wrong
...
#### IDEA-00001-R03-MED-02: Migration and rotation rewrite every item and change every id
...
#### IDEA-00001-R03-MED-03: Cloud-synced folders offer no cross-device atomicity
...
#### IDEA-00001-R03-MED-04: Starting fresh leaves plaintext items on the storage

> [!warning] Medium
> - **Confidence:** High
> - **Category:** Data
> - **Evidence:** Decision of 2026-09-15: `encrypt` may start an empty encrypted store and leave old items to `prune`. The `items` stop file needed for IDEA-00001-R03-MAJ-01 occupies the path those items live in.
...
#### IDEA-00001-R03-MED-05: Clients still holding the old data key keep writing after a rotation
...
#### IDEA-00001-R03-LOW-01: Whether typed passphrases are allowed is not yet decided

> [!note] Low
> - **Confidence:** Medium
> - **Category:** Security
> - **Evidence:** Decision of 2026-09-15: generated words are the passphrase choice. Offline guessing against the wrapped key (IDEA-00001-R02-MED-04) is resolved for generated words (≈ 77.5 bits at 64 MiB Argon2id) but not for a passphrase the user makes up.
> - **Failure scenario:** A typed option is offered and a user picks a short phrase that falls to a dictionary.
> - **Impact:** Every item readable.
> - **Mitigation or test:** Recommended: only generated words for set-up, passphrase change, and rotation. Typing is only for joining, which enters the words shown by another device.
> - **References:** RFC 9106 §4; eff.org/dice.

#### IDEA-00001-R03-LOW-02: Refusing a key file in a git work tree would break a home directory under git

> [!note] Low
> - **Confidence:** Medium
> - **Category:** Adoption
> - **Evidence:** Decision of 2026-09-15: refuse a key file inside a git work tree. Some users manage dotfiles with a git repository at `~/.git`, which puts the default `~/.config/passalong/store.key` inside a work tree.
> - **Failure scenario:** Such a user cannot use encryption at the default path, even though their repository ignores the file.
> - **Impact:** A confusing refusal at set-up.
> - **Mitigation or test:** Refuse when the key file is inside a work tree and not ignored there (`git check-ignore`). When `git` is not installed, refuse on finding a `.git` above the file. The message names both fixes: ignore the file, or set `client.key_file` elsewhere.
> - **References:** Decision log, 2026-09-15.

#### IDEA-00001-R03-LOW-03: The embedded word list needs attribution
...
#### IDEA-00001-R03-INFO-01: Some metadata stays visible
...
#### IDEA-00001-R03-INFO-02: Each client's list cache holds decrypted metadata
...
#### IDEA-00001-R03-INFO-03: The building blocks are already available
...
| E. Typed passphrases with a minimum length | Familiar | Weak phrases fall offline | High | See LOW-01 |
...
| Typed passphrases (LOW-01) | Decision | Low | High | Generated only |
| Git work tree check (LOW-02) | Risk | Low | Low | `git check-ignore` |
| Word list licence (LOW-03) | Dependency | Low | Low | Attribution header and NOTICE |
...
- [ ] Are typed passphrases allowed at all, or only generated words
      (recommended; IDEA-00001-R03-LOW-01)?
- [ ] Is refusing only un-ignored key files, via `git check-ignore`,
      acceptable (IDEA-00001-R03-LOW-02)?
...
|---|---|---|---|
| Answer 1: data key rotation in v0.1.7 | accepted | `encrypt --rotate`; one engine with migration; MED-05 added | Makes lock-out possible |
| Answer 2: offer a fresh start | accepted | Fresh start moves old items to `plain/`; MED-04 added | Large stores need not migrate |
| Answer 3: generated words | accepted | Six EFF words, ≈ 77.5 bits; LOW-03 added | High entropy without user effort |
| Answer 4: refuse a key file in a git work tree | accepted | Refuse unless git-ignored; LOW-02 added | Protects against commits without breaking dotfile repos |
| Answer 5: Argon2id at 64 MiB | accepted | Fixed at 64 MiB, t=3, p=4, stored in the header | Generated words make 2 GiB unnecessary |
| IDEA-00001-R02-MAJ-01: content key leak | resolved | Keyed content keys are part of the concept | Verified by plan tests |
| IDEA-00001-R02-MAJ-02: old and keyless clients | still-open | Now IDEA-00001-R03-MAJ-01 | Needs the spike |
| IDEA-00001-R02-MED-01: AEAD construction | still-open | Now IDEA-00001-R03-MED-01 | Implementation risk |
| IDEA-00001-R02-MED-02: migration | still-open | Widened to rotation: IDEA-00001-R03-MED-02 | Same engine |
| IDEA-00001-R02-MED-03: cloud sync | still-open | Now IDEA-00001-R03-MED-03 | Unverified |
| IDEA-00001-R02-MED-04: weak passphrase | partially-resolved | Generated words; typed option is IDEA-00001-R03-LOW-01 | Pending one decision |
| IDEA-00001-R02-MED-05: header overwrite | resolved | Exclusive creation and the join path are in the concept | Verified by plan tests |
| IDEA-00001-R02-MED-06: no revocation | resolved | Rotation in scope; new risk IDEA-00001-R03-MED-05 | Answer 1 |
| IDEA-00001-R02-LOW-01: key file name | resolved | `store.key`, `client.key_file` | Adopted |
| IDEA-00001-R02-LOW-02: version control | resolved | `*.key` ignored; work-tree refusal (LOW-02 for its edge case) | Answer 4 |
| IDEA-00001-R02-LOW-03: `--change-key` name | resolved | `encrypt --join` | Adopted |
| IDEA-00001-R02-INFO-01 to INFO-03 | superseded | Now IDEA-00001-R03-INFO-01 to INFO-03 | Still relevant |
...
1. Answer the two non-blocking questions, or accept the recommendations.
2. Write the v0.1.7 plan with the spike as STEP-01 and a stop condition if
   it fails.
3. When the spike passes, publish a final revision accepting this idea and
