...
| 2026-09-14T13:57:36Z | PLAN-00006-STEP-08 | Post-gate change requested by the user, replacing part of D-05: in `passalong choose`, `g` no longer leaves the list; it reads `Store::get_meta` and shows the `get` fields in a scrollable dialog (Up/Down/j/k, Page Up/Down, Home/End; Esc, q, g, or Enter close it). `d` then `y` deletes and lists the store again instead of only dropping the row. While the store is read or changed the status line says `Loading...` (the first listing), `Reloading...`, or `Deleting <ID>...`, because keys wait until it is done. Only Enter (load) and `c` (print) still close the list; the `Get` action is gone. Commit `feat(choose): g and d stay in the list, and reloading shows`. | choose keeps the user in the list for g and d | User |
| 2026-09-14T14:12:16Z | PLAN-00006-STEP-08 | Post-gate fix for a user report: deleting in `choose` showed the status on two lines with garbled text ("rooading"). Reproduced in a pseudo-terminal rendered with pyte: the store's info record `item deleted` went to standard error, the same terminal, mid-screen; its newline scrolled the screen and ratatui's cell diff drew `Reloading...` over it. The capture also showed `redraw_all` (ratatui `Terminal::clear`) querying the cursor position (ESC[6n), which failed where the terminal does not answer. Log records now go through `logs::writer`, which `choose`'s terminal screen holds back (`logs::hold`) until the terminal is restored, then writes in order; `redraw_all` is removed. Commit `fix(choose): hold log lines while the list is on screen`. | Clean status line; logs printed after the list closes | User |
