...
execution_updated_at: "2026-09-24T23:48:42Z"
execution_completed_at: "2026-09-19T17:03:59Z"
...
| PLAN-00010-STEP-12 | completed | 2026-09-19T16:34:56Z | 2026-09-19T17:03:59Z | `just ci` exit 0 locally on 265340c; CI green on all five jobs for 265340c (the Linux job runs `test-https`); coverage 89.34 % and coverage-full 92.75 %; timings recorded; three evidence tests added and passing | Release notes filled; hand-off below |
...
| 2026-09-19T17:03:59Z | STEP-12 | Completed:<br>• **`just ci`** ran whole on 265340c and exited 0: check, audit, the package dry run, workflow lint, the Docker SSH tests, `test-https`, the compatibility tests, the deploy test, and `coverage-full`.<br>• **Evidence tests** for three criteria that had none of their own: `each_lookup_is_one_request_to_its_own_route` (AC-03) counts the server's logged operations per `HttpStore` call. `https_a_debug_session_never_logs_the_api_key` and `debug_output_never_shows_the_api_key` (AC-08) cover a whole CLI session at `debug`, the server's log, and `Debug` of the config, the key, and the store. `a_full_workspace_names_its_quota` (AC-15) found that the server's refusal says only "the workspace is full", so `HttpStore` now answers `QUOTA_EXCEEDED` with the item's size and the workspace's usage and quota, read with `getWorkspace`. The harness gained `Settings::quota_bytes` and `TestServer::operations`.<br>• **Timings** from a release build, SSH and a server in one session, are in the release notes. At 100 encrypted items: SSH `list --nocache` 177 ms, `list` from the cache 6 ms, and `clipboard --stdin` 142 ms. A server takes 13, 6, and 15 ms. SSH matches v0.2.1 (180 ms).<br>• **Release notes** filled: tests per binary, recipes, coverage, and timings | `docs/release/v0.3.0.md` | Hand-off |
| 2026-09-24T12:40:43Z | STEP-12 | After completion, on the user's word that passalong-server v0.1.0 is released: the pin moved from `d565a28` to the tag's commit `8c75056`, three commits later with no API or behaviour change (docs, the release records, `key create`'s key-placement line now naming `passalong init`). `just test-https` passes there (39). The release binary was run once against the Docker image `joeworks/passalong-server:0.1.0` (revision label `8c75056`): init, check, the item round trip, and `encrypt` migrating an item, all passed. D-04 is closed; the developer guide, the release notes (a compatibility table), and the README (the image and the Compose walkthrough) say so | `docs/release/v0.3.0.md` Tests | Pull request and tag: user |
| 2026-09-24T23:48:42Z | Hand-off | Pull request #14 merged to `main` (54c8210). The first `v0.3.0` tag on that merge failed the release workflow's tag check, because the records had not been finalised: this release commit renames the CHANGELOG's Unreleased section to `v0.3.0` and removes the draft line from the release notes. The user re-tags its merge | Release workflow run 36074304059 | Tag `v0.3.0` on the merge of this commit: user |

### Deviations and blockers

| Timestamp (UTC) | Step | Deviation or blocker | Impact | Decision required from |
|---|---|---|---|---|
| 2026-09-19T17:03:59Z | STEP-12 | Three points:<br>• **The timing script was local**, a scratch Python driver not committed. `encrypt` needs a terminal, so it ran it on a pseudo-terminal. Its first two runs hung at the words prompt, which reads the terminal raw and so needs `\r`, and were stopped by the session for memory; the third ran clean.<br>• **AC-17's "sends the file after the commit"** cannot hold for a device without a key: after a migration commits, the workspace is sealed and that device must join first. The test lets the rewrite end by an abort and checks the file is sent then; the commit case is `https_serve_keeps_a_file_when_the_workspace_s_key_changes`.<br>• **AC-22 on Windows** was not run by this plan: CI builds, lints, and runs the non-ignored tests there, and the server tests run on Linux | Hand-off for AC-17's wording | User |
...
| 2026-09-19T17:03:59Z | STEP-11 | CI run on 265340c | Pass: all five jobs | — |
| 2026-09-19T17:03:59Z | STEP-12 | `just ci` on 265340c | Pass: exit 0 | 2145 tests passed across its recipes; coverage 89.34 %, coverage-full 92.75 % |
| 2026-09-19T17:03:59Z | STEP-12 | `just test-https` twice with the evidence tests; `just check`; timing script | Pass | 4 + 7 + 6 + 10 + CLI 12; coverage 89.30 %; eight timing rows |
...
- **Completed requirements:** REQ-01 to REQ-17
- **Incomplete requirements:** None
- **Outstanding blockers:** None in this repository. For the user:
  - AC-17's wording (STEP-12).
  - D-04 and the key-placement text were closed by passalong-server v0.1.0
    (`8c75056`), which the pin now names.
- **Review request:** Ready. Every acceptance criterion has its evidence in
  this log; AC-22's Windows part and the release tag are the user's.
