...
lint:
    ...
    # The default build of passalong-ssh has no `rsa` feature.
    cargo clippy -p passalong-ssh --all-targets -- -D warnings
...
_with-sshd CMD:
    ...
    if [ ! -f "$keys/id_rsa" ]; then
        ssh-keygen -q -t rsa -b 3072 -N "" -C passalong-it-rsa -f "$keys/id_rsa"
    fi
    # The server trusts every public key in keys/authorized.
    mkdir -p "$keys/authorized"
    cp "$keys/id_ed25519.pub" "$keys/id_rsa.pub" "$keys/authorized/"
    chmod 644 "$keys/id_ed25519.pub" "$keys/authorized/"*.pub
    ...
    export PASSALONG_IT_SSH_RSA_IDENTITY="$PWD/$keys/id_rsa"
